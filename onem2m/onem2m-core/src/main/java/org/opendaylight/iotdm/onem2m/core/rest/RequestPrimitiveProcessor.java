...
public class RequestPrimitiveProcessor extends RequestPrimitive {
    ...
    public boolean hasContent() {
        if (null != this.getPrimitiveContent()) {
            return true;
        }
        return false;
    }
    ...
    private String createParentTargetCseRelativeHierarchicalUri(Onem2mResource resource) {
        // CSE-Relative URIs must not have slash at the begin
        String parentUri = resource.getParentTargetUri();
        if (null == parentUri || parentUri.isEmpty()) {
            return resource.getName();
        } else {
            return parentUri + "/" + resource.getName();
        }
    }
    ...
    public void handleOperationCreate(ResponsePrimitive onem2mResponse) {
        ...
        if (resourceType != Onem2m.ResourceType.CSE_BASE) {
            ...
            this.setParentTargetUri(createParentTargetCseRelativeHierarchicalUri(onem2mParentResource));
    ...
    public void handleOperationDelete(ResponsePrimitive onem2mResponse) {
        ...
        // just used for stats
        ResourceContentProcessor.handleDeleteStats(this, onem2mResponse);
        if (onem2mResponse.getPrimitiveResponseStatusCode() != null) {
            return;
        }
