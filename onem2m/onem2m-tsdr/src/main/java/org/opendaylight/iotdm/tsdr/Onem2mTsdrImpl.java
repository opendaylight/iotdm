 ...
 */
public class Onem2mTsdrImpl extends Thread implements Onem2mTsdrService{
    ...
    public Onem2mTsdrImpl(RpcProviderRegistry rpcProviderRegistry){
    ...
    public void store(){
        if(this.tsdrService==null){
            this.tsdrService = this.rpcProviderRegistry.getRpcService(TsdrCollectorSpiService.class);
        }
        if(this.tsdrService==null){
            LOG.error("TSDR Service is unavailable, unable to store records.");
            synchronized (metricRecords){
                metricRecords.clear();
            }
            synchronized (logRecords){
