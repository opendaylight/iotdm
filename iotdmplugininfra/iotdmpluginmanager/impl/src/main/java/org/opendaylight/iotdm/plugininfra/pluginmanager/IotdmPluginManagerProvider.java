...
//import org.opendaylight.iotdm.plugininfra.pluginmanager.registry.IotdmRxPluginsBaseRegistry;
...
public class IotdmPluginManagerProvider implements IotdmPluginManagerService, AutoCloseable {
    ...
    public Future<RpcResult<IotdmPluginManagerIotdmPluginRegistrationsOutput>>
        ...
        return iotdmPluginManagerIotdmPluginRegistrationsImpl(input);
    }

    // TODO
    private Future<RpcResult<IotdmPluginManagerIotdmPluginRegistrationsOutput>>
                iotdmPluginManagerIotdmPluginRegistrationsImpl(IotdmPluginFilters input) {
...
//                    IotdmHandlerPlugin plugin = urlPluginEntry.getValue();
//
...
//    private boolean filterCommunicationChannels(final IotdmRxPluginsBaseRegistry registry,
...
//
//        if (null != pluginName) {
//            Optional<Map.Entry<String, IotdmHandlerPlugin>> entry =
...
//    }
    ...
    public Future<RpcResult<IotdmPluginManagerSimpleConfigClientRegistrationsOutput>>
        ...
        return this.iotdmPluginManagerSimpleConfigClientRegistrationsImpl(input);
    }

    public Future<RpcResult<IotdmPluginManagerSimpleConfigClientRegistrationsOutput>>
                                    iotdmPluginManagerSimpleConfigClientRegistrationsImpl(IotdmPluginFilters input) {
...
//
//        // Get IotdmHandlerPlugin registrations data
//        try {
...
//                                                        iotdmPluginManagerIotdmPluginRegistrationsImpl(input).get();
...
//                                        .setIotdmInterface(IotdmHandlerPlugin.class.getSimpleName())
//                                        .build());
...
//                        AtomicReference<IotdmHandlerPlugin> pluginInstance = new AtomicReference<>();
//                        IotdmPluginManager.getInstance().getMgrRegistry().registryStream().filter(registry -> {
//                            Optional<Map.Entry<String, IotdmHandlerPlugin>> p =
//                                    registry.getPluginStream()
//                                            .filter(entry ->
...
//            }
//        } catch (Exception e) {
//            LOG.error("Failed to get IotdmHandlerPlugin registrations data: {}", e);
//        }
//
//
...
//                    iotdmPluginManagerSimpleConfigClientRegistrationsImpl(input).get();
//            if (rpcResult.isSuccessful()) {
